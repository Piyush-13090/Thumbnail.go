...
const AspectRatioSelector = ({ value, onChange }: { value: AspectRatio, onChange: (ratio: AspectRatio) => void }) => {
    const iconMap = {
        ...
        '4:3': <RectangleVertical className="size-6" />,
        '1:1': <Square className="size-6" />
    } as Record<AspectRatio, React.ReactNode>

    return (
        <div className="space-y-3 dark">
            <label className="block text-sm font-medium text-zinc-100">Aspect Ratio</label>
                    ...
                    return (
                        <button
                            ...
                            className={`flex items-center gap-2 rounded-md border px-5 py-2.5 text-sm transition border-white/10 ${
                                selected ? 'bg-white/20' : 'bg-black/10 hover:bg-white/6'
                            }`}
                        >
                            {iconMap[ratio]}
