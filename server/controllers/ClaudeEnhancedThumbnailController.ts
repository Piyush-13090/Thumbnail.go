...
const enhancePromptWithClaude = async (title: string, style: string, colorScheme: string, userPrompt?: string) => {
  try {
...
Title: "${title}"
Style: ${style} (${stylePrompts[style as keyof typeof stylePrompts]})
Color Scheme: ${colorScheme} (${colorSchemeDescriptions[colorScheme as keyof typeof colorSchemeDescriptions]})
...
Generate a comprehensive, detailed prompt that will result in a professional, click-worthy thumbnail. Include specific details about composition, lighting, colors, expressions, and visual elements.`;
    ...
    return `Create a ${stylePrompts[style as keyof typeof stylePrompts]} for "${title}" with ${colorSchemeDescriptions[colorScheme as keyof typeof colorSchemeDescriptions]} color scheme. Make it professional and click-worthy.`;
  }
};
...
export const generateThumbnailWithClaude = async (req: Request, res: Response) => {
