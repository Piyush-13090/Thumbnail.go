...
const buildEnhancedPrompt = (title, style, colorScheme, userPrompt) => {
    let prompt = `Professional YouTube thumbnail: ${stylePrompts[style]} for "${title}". `;
    prompt += `${colorSchemeDescriptions[colorScheme]}. `;
    if (userPrompt) {
        prompt += `${userPrompt}. `;
    }
...
export const generateSimpleThumbnail = async (req, res) => {
    let thumbnail = null;
    try {
        ...
        const enhancedPrompt = buildEnhancedPrompt(title, style, color_scheme || 'vibrant', user_prompt);
